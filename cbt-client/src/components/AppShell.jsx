    ...
    <main className="min-h-screen bg-slate-100">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4">
          ...
          <nav className="order-3 flex w-full items-center gap-1 overflow-x-auto sm:order-2 sm:w-auto"><NavLink to="/dashboard" className={navClass}>Dashboard</NavLink>{user.role === 'super_admin' && <NavLink to="/super-admin" className={navClass}>Platform</NavLink>}{user.role === 'kurikulum' && <><NavLink to="/students" className={navClass}>Siswa</NavLink><NavLink to="/rooms" className={navClass}>Ruang</NavLink><NavLink to="/exams" className={navClass}>Ujian</NavLink><NavLink to="/question-banks" className={navClass}>Bank Soal</NavLink><NavLink to="/reports" className={navClass}>Laporan</NavLink><NavLink to="/settings/school" className={navClass}>Pengaturan Sekolah</NavLink></>}</nav>
