...
export default function StudentExamPage() {
  ...
  const submitAttempt = useCallback(async (automatic = false) => {
    ...
    try {
      const response = await api.post('/student/exam/submit')
      clearSession()
      await Swal.fire({ title: automatic ? 'Waktu ujian habis' : 'Ujian selesai', text: `Nilai Anda: ${response.data.data.score}`, icon: 'success', confirmButtonColor: '#dc2626' })
      navigate('/student-login', { replace: true })
    } catch (error) {
      submitStarted.current = false
      toast.error(getErrorMessage(error, automatic ? 'Submit otomatis gagal. Klik Selesaikan untuk mencoba lagi.' : 'Ujian gagal diselesaikan.'))
    } finally {
  ...
  useEffect(() => {
    if (data && seconds === 0) submitAttempt(true)
  }, [data, seconds, submitAttempt])
  ...
  return <main className="min-h-screen bg-slate-100"><header className="sticky top-0 z-20 border-b bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-4"><div><p className="text-xs font-bold uppercase text-brand-600">{data.exam.subject}</p><h1 className="font-bold text-slate-950">{data.exam.name}</h1></div><div className={`rounded-xl px-5 py-3 font-mono text-xl font-bold ${seconds < 300 ? 'bg-red-50 text-brand-700' : 'bg-slate-900 text-white'}`}>{minutes}:{remainingSeconds}</div></div></header><div className="mx-auto grid max-w-7xl gap-6 px-6 py-8 lg:grid-cols-[240px_1fr]"><aside className="h-fit rounded-2xl bg-white p-5 shadow-sm"><p className="font-bold text-slate-900">{user.name}</p><p className="mt-1 text-xs text-slate-500">Tersimpan {Object.keys(answers).length}/{data.questions.length}</p><div className="mt-5 grid grid-cols-5 gap-2">{data.questions.map((item, index) => <button key={item.id} onClick={() => setCurrent(index)} className={`aspect-square rounded-lg text-sm font-bold ${index === current ? 'bg-brand-600 text-white' : answers[item.id] ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>{item.number}</button>)}</div><button disabled={isSubmitting} onClick={finish} className="primary-button mt-6 w-full">{isSubmitting ? 'Mengirim...' : 'Selesaikan'}</button></aside><section className="rounded-2xl bg-white p-6 shadow-sm sm:p-8"><p className="text-sm font-bold text-brand-600">Soal {question.number}</p><p className="mt-4 text-lg font-semibold leading-8 text-slate-900">{question.text}</p><div className="mt-7 space-y-3">{question.choices.map((choice) => <button disabled={isSubmitting} key={choice.id} onClick={() => choose(question.id, choice.id)} className={`flex w-full gap-4 rounded-xl border p-4 text-left transition disabled:opacity-60 ${answers[question.id] === choice.id ? 'border-brand-500 bg-red-50 ring-2 ring-red-100' : 'border-slate-200 hover:bg-slate-50'}`}><b className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-white text-slate-700 shadow-sm">{choice.label}</b><span className="pt-1 text-slate-700">{choice.text}</span></button>)}</div><div className="mt-8 flex justify-between"><button disabled={current === 0 || isSubmitting} onClick={() => setCurrent((value) => value - 1)} className="rounded-xl border px-5 py-3 font-bold disabled:opacity-30">Sebelumnya</button><button disabled={current === data.questions.length - 1 || isSubmitting} onClick={() => setCurrent((value) => value + 1)} className="rounded-xl bg-slate-900 px-5 py-3 font-bold text-white disabled:opacity-30">Berikutnya</button></div></section></div></main>
}
